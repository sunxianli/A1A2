...
delta2=0.5;
mu=0.5;
gama1=2;%衰减系数，当其为0时，知道信息的个体是完全免疫的�?�在这里若等�?1�?
gama2=1;
ma=5;%AD网络每次连接的边�?
mb=5;
ter=40;% beta的取值个�?
...
stp=150;%时间长度
%定义矩阵
rhoG=zeros(1,ter);%节点G的密�?
rhoA1=zeros(1,ter);
rhoA2=zeros(1,ter);
load('BActivity.mat')
load('AActivity.mat')
            parfor xun = 1:ter
    beta_R=xun/(ter*gama1);
        for rep = 1:MMCA_rep
            PA2S=0.01*ones(1,N);
            PUS=0.98*ones(1,N);
            PA1G=0.01*ones(1,N);
...
xlabel('$\beta$','FontSize',15);ylabel('proportion','FontSize',15);   % 坐标轴解�?
