...
for xun = 1:mx
    ma=xun ;
        for l = 1:ter
            ...
            BETA1(xun,l)=sum(PR)/N;
            BETA2(xun,l)=sum(PI)/N;
...
%         end%(不同β取值）
    ...
    Ba=sum(BActivity)/N;
    Ba2=Ba2/N;
    EH=sqrt((Ba2-Theta_b2)*(1-BETA1(xun,l)))+Ba-Theta_b;
    beta_MMCA(xun,l)=miu/mb/EH;
        end
end
 xzhou=1:mx;
 plot(xzhou,beta_MMCA(:,1),'b-');
